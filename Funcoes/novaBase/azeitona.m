function azeitona()
    ...
    arquivoCorR = fopen(['../../Imagens/Sementes Individuais\Sementes Novas\Azeitona/corR.txt'],'wt');
    arquivoCorG = fopen(['../../Imagens/Sementes Individuais\Sementes Novas\Azeitona/corG.txt'],'wt');
    arquivoCorB = fopen(['../../Imagens/Sementes Individuais\Sementes Novas\Azeitona/corB.txt'],'wt');
    ...
    destinoEntrada = ['..\..\Imagens\Sementes Individuais\Sementes Novas\Azeitona\'];
    
    destinoSaida = ['..\..\Imagens\Sementes Individuais\Sementes Novas\Azeitona\BandasDeCores\'];
    
    mostrarBandasDeCoresVariasImagens(destinoEntrada,destinoSaida,1,qtImagens);

    for i = 1:qtImagens
        
        imAzeitona = imread(['../../Imagens/Sementes Individuais\Sementes Novas\Azeitona/' num2str(i) '.png']);
        
        CMYK = rgb2cmyk(imAzeitona);

        imBinaria = limiarAutomatico(CMYK(:,:,4));

%         figure,imshow(imBinaria),title(['Azeitona ' num2str(i) ' binaria']);

        imPreenchida =  preencherEspacos(imBinaria,8);

    %     figure,imshow(imPreenchida),title(['Azeitona ' num2str(i) ' preenchida']);

        imMenosRuidosa = bwareaopen(imPreenchida,50);

    %     figure,imshow(imMenosRuidosa),title(['Azeitona ' num2str(i) ' menos ruidos']);
...
%         imprimeEixos(Propriedades,imMenosRuidosa);
        ...
        imwrite(imMenosRuidosa,['../../Imagens/Sementes Individuais\Sementes Novas\Azeitona/m' num2str(i) '.png']);
